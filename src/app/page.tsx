...
export default function CalendarHome() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (selectedEvent) {
        // Event detail is open — go back to day modal if we came from one, else close
        setSelectedEvent(null);
        if (!selectedDateForDetails) return; // no day modal behind it — nothing else to do
        // day modal stays open (selectedDateForDetails unchanged)
  ...
  return (
    <div className={activeTheme.bg}>
      ...
      {selectedEvent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 backdrop-blur-sm p-4">
          <div className={`w-full max-w-xl rounded-2xl border p-6 relative animate-in fade-in zoom-in duration-200 ${activeTheme.modal}`}>
            <button
              onClick={() => { setSelectedEvent(null); setSelectedDateForDetails(null); }}
      ...
      {/* Day Detail Modal — time-grouped compact pill layout */}
      {selectedDateForDetails && (() => {
        const dayEvs = getSingleDayEventsForDate(selectedDateForDetails);
        ...
        const allDay   = dayEvs.filter(e => e.startTime === null);
        const morning  = dayEvs.filter(e => { const h = parseHour(e.startTime); return h !== null && h < 12; });
        const afternoon= dayEvs.filter(e => { const h = parseHour(e.startTime); return h !== null && h >= 12 && h < 17; });
        const evening  = dayEvs.filter(e => { const h = parseHour(e.startTime); return h !== null && h >= 17; });

        const slots: { label: string; emoji: string; evs: typeof dayEvs }[] = [
        ...
        const PillRow = ({ ev }: { ev: typeof dayEvs[number] }) => {
          ...
          return (
            ...
            >
              ...
              {/* Title */}
              <span className={`flex-1 text-xs font-semibold truncate group-hover:text-violet-500 transition ${activeTheme.textHeading}`}>
                {ev.title}
              </span>
              {/* Cost / Free badge */}
              {costLabel && (
                <span className={`shrink-0 px-2 py-0.5 rounded-full text-[9px] font-bold border whitespace-nowrap ${
                  isFreeTag
        ...
        return (
          <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-950/60 backdrop-blur-sm p-4">
            <div className={`w-full max-w-lg rounded-2xl border relative animate-in fade-in slide-in-from-bottom-4 duration-200 max-h-[85vh] flex flex-col ${activeTheme.modal}`}>
              {/* Header */}
              <div className={`px-6 pt-5 pb-4 border-b ${activeTheme.border} flex items-start justify-between gap-4 shrink-0`}>
                <div>
                  <h3 className={`text-base font-bold leading-tight ${activeTheme.modalTitle}`}>
                    {selectedDateForDetails.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
                  </h3>
                  <p className="text-[11px] text-slate-500 mt-0.5">
                    {dayEvs.length} {dayEvs.length === 1 ? 'activity' : 'activities'} · tap any to see details
                  </p>
                </div>
                <button
                  onClick={() => setSelectedDateForDetails(null)}
                  className={`shrink-0 p-1.5 rounded-lg transition ${activeTheme.closeBtn}`}
                >
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5 custom-scrollbar">
                {slots.map(slot => (
                  <div key={slot.label}>
                    ...
                    {/* Pill rows or empty state */}
                    {slot.evs.length === 0 ? (
                      <p className="text-[11px] text-slate-600 italic pl-1">No {slot.label.toLowerCase()} activities</p>
                    ) : (
                      <div className="space-y-1.5">
                        {slot.evs.map(ev => <PillRow key={ev.id} ev={ev} />)}
                      </div>
                    )}
                  </div>
                ))}
